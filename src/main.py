...
class MainWindow(QMainWindow):
    ...
    def show_about_dialog(self):
        # Domyślnie ustawiamy platformę jako nieznaną
        platform_name = "Nieznany"

        # Sprawdzamy, czy system to Windows
        if platform.system() == "Windows":
            platform_name = "Windows"
        # Sprawdzamy, czy system to Linux
        elif platform.system() == "Linux":
            # Jeśli tak, używamy Twojej istniejącej logiki do rozróżnienia Wayland/X11
            platform_name = "Wayland" if "wayland" in os.getenv("XDG_SESSION_TYPE", "").lower() else "X11"
        # Można dodać też inne systemy, np. macOS
