...
class ComponentSelectionDialog(QDialog):
    ...
    def _setup_ui(self):
        ...
        self.main_tab = QWidget(); self.tabs.addTab(self.main_tab, "Pliki i Skrypty")
        main_tab_layout = QVBoxLayout(self.main_tab)
        input_group = QGroupBox("Pliki wejściowe (przeciągnij i upuść na okno)"); input_layout = QVBoxLayout(input_group)
        ...
        input_layout.addWidget(self.mkv_label); input_layout.addWidget(self.mkv_button); input_layout.addWidget(self.intro_label); input_layout.addWidget(self.intro_button); input_layout.addWidget(self.subtitle_label); input_layout.addWidget(self.subtitle_button); input_layout.addWidget(self.font_label); input_layout.addWidget(self.font_button)
        main_tab_layout.addWidget(input_group)
        script_group = QGroupBox("Skrypty"); script_layout = QVBoxLayout(script_group)
        self.ffmpeg_radio = QRadioButton("Użyj tylko FFmpeg (hardsub)"); self.mkvmerge_ffmpeg_radio = QRadioButton("Użyj mkvmerge i FFmpeg"); self.mkvmerge_radio = QRadioButton("Użyj tylko mkvmerge (remux)"); self.ffmpeg_intro_radio = QRadioButton("FFmpeg + Wstawka (intro)")
        self.mkvmerge_radio.setChecked(True)
        self.button_group = QButtonGroup(); self.button_group.addButton(self.ffmpeg_radio, 1); self.button_group.addButton(self.mkvmerge_ffmpeg_radio, 2); self.button_group.addButton(self.mkvmerge_radio, 3); self.button_group.addButton(self.ffmpeg_intro_radio, 4)
        script_layout.addWidget(self.ffmpeg_radio); script_layout.addWidget(self.mkvmerge_ffmpeg_radio); script_layout.addWidget(self.mkvmerge_radio); script_layout.addWidget(self.ffmpeg_intro_radio)
        main_tab_layout.addWidget(script_group)
        file_options_group = QGroupBox("Opcje Pliku"); file_options_layout = QHBoxLayout(file_options_group)
        self.info_button = QPushButton("Pokaż informacje o pliku MKV"); self.info_button.setToolTip("Otwiera nowe okno ze szczegółami pliku wideo"); self.info_button.setEnabled(False)
        file_options_layout.addWidget(self.info_button); main_tab_layout.addWidget(file_options_group); main_tab_layout.addStretch()
        self.output_tab = QWidget(); self.tabs.addTab(self.output_tab, "Ustawienia Wyjściowe")
        output_tab_layout = QVBoxLayout(self.output_tab)
        self.output_disabled_label = QLabel("Funkcje tej zakładki nie są dostępne dla tej opcji."); self.output_disabled_label.setAlignment(Qt.AlignmentFlag.AlignCenter); self.output_disabled_label.setStyleSheet("font-style: italic; color: #888888;"); self.output_disabled_label.setVisible(False)
        output_tab_layout.addWidget(self.output_disabled_label)
        self.output_group = QGroupBox("Lokalizacja i nazwa pliku końcowego"); output_layout = QVBoxLayout(self.output_group)
        self.custom_output_checkbox = QCheckBox("Użyj niestandardowych ustawień wyjściowych"); output_layout.addWidget(self.custom_output_checkbox)
        location_layout = QHBoxLayout(); self.output_dir_label = QLabel("Folder wyjściowy:")
        self.output_dir_edit = QLineEdit(); self.output_dir_edit.setPlaceholderText("Domyślnie folder pliku źródłowego")
        self.output_dir_button = QPushButton("Przeglądaj..."); location_layout.addWidget(self.output_dir_label); location_layout.addWidget(self.output_dir_edit); location_layout.addWidget(self.output_dir_button)
        output_layout.addLayout(location_layout)
        name_layout = QHBoxLayout(); self.output_name_label = QLabel("Nazwa pliku:")
        self.output_name_edit = QLineEdit(); self.output_name_edit.setPlaceholderText("Domyślnie generowana automatycznie")
        name_layout.addWidget(self.output_name_label); name_layout.addWidget(self.output_name_edit); output_layout.addLayout(name_layout); output_tab_layout.addWidget(self.output_group)
        self.suffix_group = QGroupBox("Modyfikacja Nazwy Pliku"); suffix_layout = QVBoxLayout(self.suffix_group)
        top_suffix_layout = QHBoxLayout(); self.suffix_combo = QComboBox(); self.suffix_combo.setToolTip("Wybierz końcówkę do dodania do nazwy pliku")
        self.apply_suffix_button = QPushButton("Dodaj Końcówkę"); self.restore_name_button = QPushButton("Przywróć Domyślną")
        ...
        bottom_suffix_layout = QHBoxLayout(); self.custom_suffix_edit = QLineEdit(); self.custom_suffix_edit.setPlaceholderText("Wpisz własną końcówkę (np. _final)")
        self.add_suffix_button = QPushButton("Dodaj"); self.remove_suffix_button = QPushButton("Usuń")
        ...
        suffix_layout.addLayout(bottom_suffix_layout); output_tab_layout.addWidget(self.suffix_group); output_tab_layout.addStretch()
        self.advanced_tab = QWidget(); self.tabs.addTab(self.advanced_tab, "Opcje Zaawansowane")
        advanced_tab_layout = QVBoxLayout(self.advanced_tab)
        ffmpeg_group = QGroupBox("Ustawienia enkodera"); ffmpeg_layout = QVBoxLayout(ffmpeg_group)
        self.ffmpeg_script_label = QLabel("Wybierz skrypt FFmpeg (dla opcji z FFmpeg):")
        self.script1_radio = QRadioButton("CPU (CRF)"); self.script2_radio = QRadioButton("GPU (Nvidia, Bitrate)"); self.script1_radio.setChecked(True)
        self.script_button_group = QButtonGroup(); self.script_button_group.addButton(self.script1_radio, 1); self.script_button_group.addButton(self.script2_radio, 2)
        bitrate_layout = QHBoxLayout(); self.bitrate_label = QLabel("Bitrate (Mbps):"); self.bitrate_spinbox = QSpinBox(); self.bitrate_spinbox.setRange(1, 100); self.bitrate_spinbox.setValue(8)
        bitrate_layout.addWidget(self.bitrate_label); bitrate_layout.addWidget(self.bitrate_spinbox)
        ffmpeg_layout.addWidget(self.ffmpeg_script_label); ffmpeg_layout.addWidget(self.script1_radio); ffmpeg_layout.addWidget(self.script2_radio); ffmpeg_layout.addLayout(bitrate_layout)
        advanced_tab_layout.addWidget(ffmpeg_group)
        other_group = QGroupBox("Inne"); other_layout = QVBoxLayout(other_group)
        self.debug_checkbox = QCheckBox("Debug Mode", self); self.import_button = QPushButton("Importuj zadania z pliku TXT", self)
        self.help_button = QToolButton(self); self.help_button.setIcon(self.style().standardIcon(QStyle.StandardPixmap.SP_DialogHelpButton))
        ...
        advanced_tab_layout.addWidget(other_group); advanced_tab_layout.addStretch()
    ...
    def update_ui_state(self):
        ...
        self.output_group.setVisible(not is_output_tab_disabled); self.suffix_group.setVisible(not is_output_tab_disabled); self.output_disabled_label.setVisible(is_output_tab_disabled)
        needs_subtitles = selected_id in [1, 2, 3]; needs_intro = selected_id == 4
        ...
        self.output_name_edit.setEnabled(use_custom_output); self.suffix_group.setEnabled(use_custom_output)
        ffmpeg_encoder_id = self.script_button_group.checkedId(); ffmpeg_encoder_options_enabled = selected_id in [1, 2]; is_gpu_selected = ffmpeg_encoder_options_enabled and ffmpeg_encoder_id == 2
        ...
        self.output_name_edit.setText(self._generate_default_output_name()); self._validate_inputs()
    ...
    def dropEvent(self, event):
        ...
        self.update_file_labels(); self._validate_inputs(); event.acceptProposedAction(); self.update_ui_state()
